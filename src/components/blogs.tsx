            ...
            {blogs.map((blog) => (
                <Link href={blog.url} key={blog.url}>
                    <div className="p-4 md:space-x-4 border rounded-lg shadow-md cursor-pointer flex flex-col md:flex-row h-full">
                        <div>
                            <h2 className="text-2xl font-bold mb-1 dark:text-white">{blog.title}</h2>
                            <h3 className="text-lg text-gray-600 mb-1 dark:text-gray-300">{blog.subtitle}</h3>
                            ...
                            <p className="text-gray-700 mb-4 md:mb-0 dark:text-gray-300">{truncateText(blog.brief, 150)}</p>
                        </div>
                        {blog.coverImage?.url && (
                            <div >
                                <Image
